...
% ver. 1: 21th Nov. 2013
% Ver. 1.1: 5th 2016 (fix display error)

try 
    % initialization: Experiment, Input Device, 
    blk_trials = 30;
    exp = CExp(5,[2 3],'blockFactors',2,'blockRepetition',4); % target present/absent ; set size (3)
    ...
    for iTrl=1: exp.maxTrls
        ...
        for iframe = 1: v.sec2frames(5) % maximum 5 seconds display
            ...
            % search display 
            v.dispItems(xys, items, repmat(itemSizes,c_size,1),rotations);
